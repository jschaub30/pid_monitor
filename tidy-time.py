...
def main(argList):
    ...
    print(header)
    fields0 = m.fields()  # Store current fields to compare against later

    m = parse_time(m, run_fn)
    # CSV output will be incorrect if you add another field without initializing
...
def parse_time(m, fn):
    ...
    m.cpu_pct = blob.split('Percent of CPU this job got: ')[1].split('\n')[0].strip('%')
    print m.row()
